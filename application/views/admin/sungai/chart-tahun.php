    ...
    <div class="col-md-12">
      ...
      <div class="box box-success">
        <div class="box-header with-border">
          <form class="box-title" method="get" action="<?php echo site_url('adm_sungai/chart_tahun') ?>">
            <select name="tahun1">
              <option value="">- Pilih Tahun -</option>
              <?php
              foreach ($tahun->result() as $t) {
                $s = '';
                if($t->tahunuji_sungai==$this->input->get('tahun1')) {
                  $s = 'selected';
                }
            ...
            <select name="tahun2">
              ...
              foreach ($tahun->result() as $t) {
                $s = '';
                if($t->tahunuji_sungai==$this->input->get('tahun2')) {
                  $s = 'selected';
                }
                echo '<option value="'.$t->tahunuji_sungai.'" '.$s.'>'.$t->tahunuji_sungai.'</option>';
              }
            ...
            <select name="sungai">
              ...
              foreach ($lokasi->result() as $l) {
                $s = '';
                if($l->lokasiuji_sungai_id==$this->input->get('sungai')) {
                  $s = 'selected';
                }
                echo '<option value="'.$l->lokasiuji_sungai_id.'" '.$s.'>'.$l->nama_lokasi.'</option>';
              }
              ?>
            </select>
          ...
          datasets: [
            {
              ...
              <?php
                for($i=$tahun1; $i<=$tahun2; $i++){
                  echo hsungai(array('periode'=>0,'a.lokasiuji_sungai_id'=>$sungai,'par_sungai_situ_id'=>$par,'tahunuji_sungai'=>$i));
                  echo ',';
                }
              ?>
              ]
            },
            {
              ...
              <?php
                for($i=$tahun1; $i<=$tahun2; $i++){
                  echo hsungai(array('periode'=>1,'a.lokasiuji_sungai_id'=>$sungai,'par_sungai_situ_id'=>$par,'tahunuji_sungai'=>$i));
                  echo ',';
                }
              ?>
              ]
