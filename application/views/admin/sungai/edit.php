...
echo form_open_multipart('adm_sungai/edit_lokasiuji');
echo form_hidden('id',$row['lokasiuji_sungai_id']);
?>
<div class="box">
  <div class="box-header text-right">
      <button type="submit" class="btn btn-sm bg-orange"><i class="fa fa-save"></i> Simpan</button>
      <button type="reset" class="btn btn-sm btn-default"> Reset</button>
      <a href="<?php echo site_url('adm_sungai/lokasiuji')?>" class="btn btn-sm bg-grey">Kembali</a>
  </div>
</div>
<div class="row">
  ...
  <div class="col-md-6">
    <div class="box">
      <div class="box-body">
        <div class="form-group">
          <label>sungai*</label>
          <select name="sungai_id" class="form-control">
            <option value="">- Pilih -</option>
            <?php
            foreach ($sungai->result() as $pl) {
              $s = '';
              if($row['sungai_id']==$pl->sungai_id) {
                $s = 'selected';
              }
              echo '<option value="'.$pl->sungai_id.'" '.$s.'>'.$pl->nama_sungai.'</option>';
            }
            ?>
          </select>
          <p class="help-block text-red"><?php echo form_error('sungai_id', ' ')?></p>
        </div>
        ...
        <div class="form-group">
          ...
          <select name="lokasi_uji" class="form-control">
            <?php
            $i = 0;
            foreach ($lokasiuji as $l) {
              $sq = '';
              if($row['lokasi_uji']==$l) {
                $sq = 'selected';
              }
              echo '<option value="'.$l.'" '.$sq.'>'.$l.'</option>';
            $i++;
            }
            ?>
          </select>
       ...
       <option value="small">small</option>
		...
		<p><b><font face="Arial">Data Lokasi Pengujian Sungai</font></b></p>
