...
<table class="table table-bordered table-hover" id="list">
	...
	<tbody>
		...
		<tr>
			...
			<td><?php echo $dt->hasil_uji ?></td>
			<td><?php echo $dt->ket_akhir ?></td>
			<td>
				<input type="hidden" value="<?php echo $dt->par_sungai_situ_id ?>" id="parameter<?php echo $no ?>">
		...
		<script type="text/javascript">
			$("#hapushasiluji<?php echo $no ?>").click(function(){
				...
				var tanto = "parameter="+parameter+"&tahun="+tahunuji+"&periode="+periode+"&lokasi="+lokasiuji;
				$("#loading").show();
				$.ajax({
				  type: "POST",
			      url: "<?php echo site_url('adm_sungai/hapushasiluji') ?>",
			      cache: false,
			      data: tanto, 
	...
	<div class="col-md-3">
		<select name="parameter" class="form-control" id="parameter">
			...
			foreach ($parameter->result() as $row) {
				echo "<option value='".$row->par_sungai_situ_id."'>".$row->parameter."</option>";
			}
			?>
		</select>
...
$("#tmbhasiluji").submit(function(e) {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('adm_sungai/tmbhasiluji') ?>",
      contentType: false,
      cache: false,
      processData: false,
