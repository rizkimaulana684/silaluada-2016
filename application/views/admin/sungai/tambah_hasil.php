        ...
        <form method="post" id="cekdatapengujian">
          ...
          <div class="col-md-12 form-group">
            <select name="periode" class="form-control" id="periode">
              ...
              <?php
              foreach ($periode as $s => $p) {
                echo '<option value="'.$s.'">'.$p.'</option>';
              }
              ?>
            </select>
            <p class="help-block text-red"><?php echo form_error('lokasiuji', ' ')?></p>
          ...
          <div class="col-md-12">
            ...
            <a href="<?php echo site_url('adm_sungai') ?>" class="btn btn-default">Kembali</a>
          </div>
        </form>
        </div>
...
<script type="text/javascript">
  $("#cekdatapengujian").submit(function(e) {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('adm_sungai/cekdatapengujian') ?>",
  ...
  $("#uploadcsv").submit(function(e) {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('adm_sungai/importdata') ?>",
      enctype: 'multipart/form-data',
      contentType: false,
      cache: false,
      processData: false,
