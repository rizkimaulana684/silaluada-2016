...
echo form_open_multipart('adm_situ/edit_lokasiuji');
echo form_hidden('id',$row['lokasiuji_situ_id']);
?>
<div class="box">
  <div class="box-header text-right">
      <button type="submit" class="btn btn-sm bg-orange"><i class="fa fa-save"></i> Simpan</button>
      <button type="reset" class="btn btn-sm btn-default"> Reset</button>
      <a href="<?php echo site_url('adm_situ/lokasiuji')?>" class="btn btn-sm bg-grey">Kembali</a>
  </div>
</div>
<div class="row">
  ...
  <div class="col-md-6">
    <div class="box">
      <div class="box-body">
        <div class="form-group">
          <label>Situ*</label>
          <select name="situ_id" class="form-control">
            <option value="">- Pilih -</option>
            <?php
            foreach ($situ->result() as $pl) {
              $s = '';
              if($row['situ_id']==$pl->situ_id) {
                $s = 'selected';
              }
              echo '<option value="'.$pl->situ_id.'" '.$s.'>'.$pl->nama_situ.'</option>';
            }
            ?>
          </select>
          <p class="help-block text-red"><?php echo form_error('situ_id', ' ')?></p>
        </div>
