...
<table class="table table-bordered table-hover" id="list">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<input type="hidden" value="<?php echo $dt->par_cerobong_id ?>" id="parameter<?php echo $no ?>">
		...
		<script type="text/javascript">
			$("#hapushasiluji<?php echo $no ?>").click(function(){
				...
				$.ajax({
				  type: "POST",
			      url: "<?php echo site_url('adm_cerobong/hapushasiluji') ?>",
			      cache: false,
			      data: tanto, 
	...
	<div class="col-md-3" style="padding-right:5px;">
		<select name="parameter" class="form-control" id="parameter">
			...
			foreach ($parameter->result() as $row) {
				echo "<option value='".$row->par_cerobong_id."'>".$row->parameter."</option>";
			}
			?>
		</select>
...
$("#tmbhasiluji").submit(function(e) {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('adm_cerobong/tmbhasiluji') ?>",
      contentType: false,
      cache: false,
      processData: false,
