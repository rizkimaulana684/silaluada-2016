...
echo form_open_multipart('adm_limbah_cair/edit_lokasiuji');
echo form_hidden('id',$row['lokasiuji_limbah_cair_id']);
?>
<div class="box">
      ...
      <div class="box-body">
        <div class="form-group">
          ...
          <select name="limbah_cair_id" class="form-control">
            ...
            <?php
            foreach ($limbah_cair->result() as $pl) {
              $s = '';
              if($row['limbah_cair_id']==$pl->limbah_cair_id) {
                $s = 'selected';
              }
              echo '<option value="'.$pl->limbah_cair_id.'" '.$s.'>'.$pl->nama_objekuji.'</option>';
            }
            ?>
          </select>
