  ...
  <ol class="breadcrumb">
      ...
      <select name="tahun1" id="tahun1">
              ...
              foreach ($tahun->result() as $t) {
                echo '<option value="'.$t->tahunuji_sumur.'" >'.$t->tahunuji_sumur.'</option>';
              }
              ?>
            </select>
            <span>s/d</span>
            ...
            <select name="sungai" id="lksumur">
              <option value="">- Pilih Lokasi -</option>
              <?php
              foreach ($lokasi->result() as $l) {
                echo '<option value="'.$l->lokasiuji_sumur_id.'" >'.$l->nama_lokasi.'</option>';
              }
              ?>
            </select>
            <select name="parameter" id="parameter">
              ...
              foreach ($parameter->result() as $t) {
                echo '<option value="'.$t->par_sumur_id.'" >'.$t->parameter.'</option>';
              }
              ?>
            </select>
...
<script>
  $("#proses").click(function() {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('trend/trend_sumur')?>",
