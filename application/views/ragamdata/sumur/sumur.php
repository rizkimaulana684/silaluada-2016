      ...
      <select id="tahunuji" name="tahunuji">
        ...
        if($tahunuji->num_rows()>0) {
          foreach ($tahunuji->result() as $rows) {
            ?>
            <option value="<?php echo $rows->tahunuji_sumur ?>"><?php echo $rows->tahunuji_sumur ?></option>
            <?php
          }
        }
        ?>
...
<script>
  $("#proses").click(function() {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('ragam_data/report_sumur')?>",
  ...
  $("#tahunuji").change(function() {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('ragam_data/pilih_tahun_sumur')?>",
      data: "tahunuji="+thn,
      success: function(data) {
        $("#loading").hide();
        $("#lokasi").html(data);
