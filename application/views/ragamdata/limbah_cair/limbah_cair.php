      ...
      <select id="tahunuji" name="tahunuji">
        ...
        if($tahunuji->num_rows()>0) {
          foreach ($tahunuji->result() as $rows) {
            ?>
            <option value="<?php echo $rows->tahunuji_limbah_cair ?>"><?php echo $rows->tahunuji_limbah_cair ?></option>
            <?php
          }
        }
        ?>
...
<script>
  $("#proses").click(function() {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('ragam_data/report_limbah_cair')?>",
  ...
  $("#tahunuji").change(function() {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('ragam_data/pilih_tahun_lc')?>",
      data: "tahunuji="+thn,
      success: function(data) {
        $("#loading").hide();
        $("#lokasi").html(data);
