  ...
  <ol class="breadcrumb">
      ...
      <select name="tahun1" id="tahun1">
              ...
              foreach ($tahun->result() as $t) {
                echo '<option value="'.$t->tahunuji_situ.'" >'.$t->tahunuji_situ.'</option>';
              }
              ?>
            </select>
            <span>s/d</span>
            ...
            <select name="sungai" id="lksitu">
              <option value="">- Pilih Lokasi -</option>
              <?php
              foreach ($lokasi->result() as $l) {
                echo '<option value="'.$l->lokasiuji_situ_id.'" >'.$l->nama_lokasi.'</option>';
              }
              ?>
            </select>
...
<script>
  $("#proses").click(function() {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('trend/trend_situ')?>",
