  ...
  <ol class="breadcrumb">
      ...
      <select name="tahun1" id="tahun1">
              ...
              foreach ($tahun->result() as $t) {
                echo '<option value="'.$t->tahunuji_cerobong.'" >'.$t->tahunuji_cerobong.'</option>';
              }
              ?>
            </select>
            <span>s/d</span>
            ...
            <select name="sungai" id="lkcerobong">
              <option value="">- Pilih Lokasi -</option>
              <?php
              foreach ($lokasi->result() as $l) {
                echo '<option value="'.$l->lokasiuji_cerobong_id.'" >'.$l->nama_lokasi.'</option>';
              }
              ?>
            </select>
            <select name="parameter" id="parameter">
              ...
              foreach ($parameter->result() as $t) {
                echo '<option value="'.$t->par_cerobong_id.'" >'.$t->parameter.'</option>';
              }
              ?>
            </select>
...
<script>
  $("#proses").click(function() {
    ...
    var thn = $("#tahun1").val();
    var thn2 = $("#tahun2").val();
    var s = $("#lkcerobong").val();
    var ad = $("#parameter").val();
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('trend/trend_cerobong')?>",
