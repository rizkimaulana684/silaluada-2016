  ...
  <ol class="breadcrumb">
      ...
      <select name="tahun1" id="tahun1">
              ...
              foreach ($tahun->result() as $t) {
                echo '<option value="'.$t->tahunuji_ambien.'" >'.$t->tahunuji_ambien.'</option>';
              }
              ?>
            </select>
            <span>s/d</span>
            ...
            <select name="sungai" id="lkambien">
              <option value="">- Pilih Lokasi -</option>
              <?php
              foreach ($lokasi->result() as $l) {
                echo '<option value="'.$l->lokasiuji_ambien_id.'" >'.$l->nama_lokasi.'</option>';
              }
              ?>
            </select>
            <select name="parameter" id="parameter">
              ...
              foreach ($parameter->result() as $t) {
                echo '<option value="'.$t->par_ambien_id.'" >'.$t->parameter.'</option>';
              }
              ?>
            </select>
...
<script>
  $("#proses").click(function() {
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('trend/trend_ambien')?>",
