    ...
    function load_map() {
      ...
      downloadUrl("<?php echo site_url('mapsxml/'.$linked); ?>", function(data) {
        ...
        for (var i = 0; i < markers.length; i++) {
          var mata_air_id = markers[i].getAttribute("mata_air_id");
          var icon_map = '<?php echo base_url("assets/gmaps/images/icon-mata_air.png")?>';
 
          var point = new google.maps.LatLng(
              parseFloat(markers[i].getAttribute("lat")),
              parseFloat(markers[i].getAttribute("lng")));
          var html = "<?php echo site_url('ragamdata/mata_air') ?>/"+mata_air_id;
